...
function shouldPreserveScrollBetween(oldPathname, newPathname) {
  ...
  if (
    ...
    (countSlashes(oldPathname) === 3 &&
      ...
      // /stuff/ === /stuff/
      oldPathname.substr(oldPathname.substr(1).indexOf('/') + 1) === newPathname.substr(newPathname.substr(1).indexOf('/') + 1))
  ) {
    return true;
  }
  return false;
...
require('gatsby-remark-autolink-headers/gatsby-browser').shouldUpdateScroll = function shouldUpdateScroll({ prevRouterProps, routerProps }) {
