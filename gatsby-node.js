      ...
      graphql(
        ...
        _.each(defaultLangPosts, (post, index) => {
          ...
          _.each(otherLangPosts, post => {
            ...
            // Record which links to internal posts have translated versions into this language.
            // We'll replace them before rendering HTML.
            let translatedLinks = [];
            ...
            maybeAbsoluteLinks.forEach(link => {
              if (allSlugs.has(link)) {
                if (allSlugs.has('/' + langKey + link)) {
                  // This is legit an internal post link, and it has been already translated.
